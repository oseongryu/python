...
if __name__ == "__main__":
        ...
        with pymssql.connect('localhost', 'sa', '1234', "NSK") as conn_c2:
            with pymssql.connect('localhost', 'sa', '1234', "NSK") as conn:
                with conn.cursor(as_dict=True) as cursor:
                        cursor.execute(" SELECT MKT_CD FROM TB_STK_MKT WHERE MKT_CLSS_CD = 0 ")
                        i = 0
                        for row in cursor:
                            i = i + 1
                            company_code = row['MKT_CD']
                            for x in range(1, 30):
